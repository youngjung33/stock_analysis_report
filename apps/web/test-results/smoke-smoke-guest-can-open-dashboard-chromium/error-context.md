...
- Name: smoke.spec.ts >> smoke >> guest can open dashboard
- Location: ..\..\test\e2e\smoke.spec.ts:21:7
...
Call log:
  - Expect "toHaveURL" with timeout 5000ms
    13 × unexpected value "http://127.0.0.1:3000/login"

```

```yaml
- heading "SAR Portfolio" [level=1]
- paragraph: 로그인 후 거래와 시세를 관리하세요.
- paragraph: 소셜 로그인은 OAuth 클라이언트 설정 후 사용할 수 있습니다.
- text: 또는 아이디로 아이디
- textbox "아이디"
- text: 비밀번호
- textbox "비밀번호"
...
- paragraph:
  - link "비밀번호를 잊으셨나요?":
    - /url: /forgot-password
- paragraph: 비회원으로 둘러보기
- paragraph: 서버에 저장되지 않으며, 이 브라우저 탭을 닫으면 데이터가 사라집니다.
- button "입장 중..." [disabled]
- alert
```
