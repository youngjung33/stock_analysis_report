...
export function MobileLoginPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col justify-center bg-background px-4 py-8 pb-24">
      <form
        noValidate
      ...
      >
        <div>
          <h1 className="text-xl font-semibold tracking-tight">{APP_BRAND.name}</h1>
        ...
        <label className="block">
          ...
          <div className="mt-1 flex gap-2">
            <input
              className="min-w-0 flex-1 rounded-lg border border-border-strong bg-muted px-3 py-2.5 text-foreground outline-none focus:border-primary"
            ...
            {isRegister && (
              <button
                ...
                className="shrink-0 rounded-lg border border-border-strong px-2.5 py-2 text-xs text-muted-foreground hover:bg-accent hover:text-foreground disabled:opacity-50"
              >
                확인
              </button>
            )}
        ...
        {isRegister && (
          <label className="block">
            <span className="text-xs text-muted-foreground">이메일 (선택)</span>
            <input
              type="text"
              inputMode="email"
              className="mt-1 w-full rounded-lg border border-border-strong bg-muted px-3 py-2.5 text-foreground outline-none focus:border-primary"
              value={screen.email}
        ...
        <div className="rounded-xl border border-border bg-muted/50 p-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            서버에 저장되지 않으며, 탭을 닫으면 데이터가 사라집니다.
          </p>
          <button
            type="button"
            onClick={screen.handleGuestLogin}
            disabled={screen.loading || screen.guestLoading || screen.oauthLoading}
            className="mt-3 w-full rounded-lg border border-border-strong py-2.5 text-sm font-medium text-muted-foreground hover:bg-accent hover:text-foreground disabled:opacity-50"
          >
